...
function setEventListeners() {
    ...
    function clickAddToChart(event) {
        ...
        alert("Seçtiğiniz kitap başarılı bir şekilde sepete eklenmiştir.");
    }
    ...
    function clickAddFavoriteListButton(event) {
        ...
        var filterData = bookData.filter(item=>item.bookId==bookId);
        if(filterData.length > 0)
            favoriteList.push(filterData[0]);
        alert("Seçtiğiniz kitap başarılı bir şekilde favori listesine eklenmiştir.");
    }
    ...
    function clickAddRapidBuyButton(event) {
        ...
        var filterData = bookData.filter(item=>item.bookId==bookId);
        if(filterData.length > 0)
            rapidList.push(filterData[0]);
        
        setPopupTitle('Hızlı Satın Alma');
...
function openPopup(popupName) {
    ...
    if(popupName == "FavoriteList")
        popupList = favoriteList;
    else if(popupName == "ChartList")
        popupList = chartList;
    else
        popupList = rapidList;
    
    if ((table == null || table == undefined )&& popupList.length > 0)
    {
        createTable();
        insertRowToTable(popupList);
        insertRowToTotalAmount(popupList);
    }
    else if(popupList.length > 0)
    {
        insertRowToTable(popupList);
        insertRowToTotalAmount(popupList);
    }
...
function insertRowToTable(tableData)
{
    ...
    $(tableData).each(function (i, rowData) {
        ...
        $(rowData).each(function (j, cellData) {
            row.append($('<td class="tdImg">'+'<img class="tableImg" src='+cellData.bookUrl+'>'+'</td>'));
...
function clearList()
{
    if(currentPopupName == "FavoriteList")
        clearFavoriteList();
    else
        clearChartList();
...
function clickSearchButton()
{
    ...
    if(input.value.length < 3)
        alert("Arama yapabilmek için en az 3 harf girişi yapılmalıdır.");
    else
        matchingList = getMatchingDataForFilter(input);
}

function getMatchingDataForFilter(input)
{
    var allBookList = [];
    ...
    var matchingList = allBookList.filter(item=>
    {
        var bookName = stringConvert2UpperCase(item.bookName);
        value        = stringConvert2UpperCase(value);

        if(bookName.search(value) != -1)
            return true;
        else
            return false;
    })
...
function stringConvert2UpperCase(string)
{
    var result = new String(string).toUpperCase();
    return result;
}